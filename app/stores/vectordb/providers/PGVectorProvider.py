...
from ..VectorDBEnums import (DistanceMethodEnums, PgvectorDistanceMethodEnums,
                             PgvectorTableSchemeEnums, PgvectorIndexTypesEnums, VectorDBEnums)
...
class PGVectorProvider(VectorDBInterface):
    ...
    async def create_vector_index(self, collection_name: str,
                                  inex_type: str = PgvectorIndexTypesEnums.HNSW.value):
        ...
        async with self.db_client() as session:
            async with session.begin():
                ...
                create_idx_sql = sql_text(
                                            ...
                                            f'USING {index_name}({PgvectorTableSchemeEnums.VECTOR.value} {self.distance_method})'
                                          )
    ...
    async def reset_vector_index(self, collection_name: str,
                                        index_type: str = PgvectorIndexTypesEnums.HNSW.value) -> bool:
    ...
    async def insert_one(self, collection_name: str, text: str, vector: list,
        ...
        is_collection_existed = await self.is_collection_existed(collection_name=collection_name)
        if not is_collection_existed:
            self.logger.info(f"can not insert into a not existing collection:  {collection_name}")
            return False

        if not record_id:
            self.logger.info(f"can not insert without a chunk_id:  {collection_name}")
            return False
    ...
    async def insert_many(self, collection_name: str, text: list, vector: list,
                          metadata: list = None,
                          record_ids: str = None,
                          batch_size: int = 50,):
        ...
        async with self.db_client() as session:
            async with session.begin():
                for i in range(0, len(text), batch_size):
                    ...
                    batch_record_id = record_ids[i:i + batch_size]
    ...
    async def search_by_vector(self, collection_name: str, vector: list, limit: int)->List[RetrievedDocuments]:
